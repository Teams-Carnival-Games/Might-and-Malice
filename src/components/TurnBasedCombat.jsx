...
const [ message, setMessage ] = useState("This application only works in a Teams meeting");
...
const { unityProvider, sendMessage, addEventListener, removeEventListener } = useUnityContext({
  ...
  const handlePlayerOneTurn = useCallback(async () => {
    try {
      console.log("Heard event: END OF PLAYER 2 ATTACK. Triggering callback to move to player 1 turn.");
  ...
  const handlePlayerTwoTurn = useCallback(async () => {
    try {
      console.log("Heard event: END OF PLAYER 1 ACTION. Triggering callback to move to player 2 turn.");
...
}, [addEventListener, removeEventListener, giveInitialState, handlePlayerOneAttackReact]);
...
useEffect(() => {
  ...
  app.initialize().then(async () => {
    try {
      ...
      const playerValues = Object.entries(playersObject).map(([key, value]) => `${key}: ${value}`);
      ...
      FluidService.onNewPeopleData(async (people) => {
        console.log("Changes made to people list in Fluid container: ", people);
  ...
  async function handlePeopleChange() {
    ...
    console.log("Handle people change got: ", newObject, "from Fluid container.")
    const newValues = Object.entries(newObject).map(([key, value]) => `${key}: ${value}`);
    setPeople(newValues);
  ...
  async function handleStateChange() {
    ...
    switch (newState) {
      ...
      case 7:
        ...
        console.log("HSC received", chance, "from Fluid and will pass the value", chance["chance"], "to Unity.");
        console.log("At time of HSC case 7 start, the signature on the chance map is: ", chance["signature"]);
        ...
        if (updatedChance["gladius-sign"] === "signed" && updatedChance["magilax-sign"] === "signed") {
          const newChance = getRandomInt(1, 10);
          console.log("HSC will now update the Fluid Chance map with the new value, ", newChance);
      ...
      case 8:
        ...
        if (mdatedChance["gladius-sign"] === "signed" && mdatedChance["magilax-sign"] === "signed") {
  ...
  async function getBattleStateLS() {
    ...
    console.log("getBattleStateLS got: ", gameInfo[0]["battle-state"], " from Fluid SharedMap");
  ...
  if (!ready) {
    ...
    if (frame == "Side panel") {
      ...
      return (
        ...
        <button onClick={() => removeFromGame()}> - Leave the fight </button>}
        {sharing === false && <button onClick={() => shareToStage()}>Share to stage</button>}
      </div>
      );
      ...
      return (
        <div className="join">
          ...
          <div className="unity-container"><Unity unityProvider={unityProvider} style={{width: 1000, height: 530}}/></div>
        </div>
      );
    }
  }
